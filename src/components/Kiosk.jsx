      ...
      <div className="modal-dialog" role="document">
        <div className="modal-content">
          ...
          <div className="modal-footer">
            ...
            <button type="button" className="btn btn-danger" onClick={() => dispatch(postActions(id, command.machine_id, command.machine_name, command.action_id, command.action_name,inputValues[0]))} data-dismiss="modal">Continue</button>
          </div>
        </div>
      </div>
    </div>
...
const ActionButtons = () => {
  ...
  const handleNotConfirmClick=(command,inputValues)=>{
    dispatch(postActions(id, command.machine_id, command.machine_name, command.action_id, command.action_name,inputValues))
  }
  const closeModal = () => {
  ...
  commands.forEach(command => {
    const { machine_id, machine_name, action_id, machine_type, action_name, confirm, name,params } = command;
    if (!groupedCommands[machine_type]) {
      groupedCommands[machine_type] = [];
    }
    groupedCommands[machine_type].push({
      machine_id,
      machine_name,
      action_id,
      action_name,
  ...
  const newcommands = Object.entries(groupedCommands).map(([machine_type, commands]) => (
    <div key={machine_type}>
      <h5>machine_type: {machine_type}</h5>
          ...
          <div key={buttonIndex} style={{ display: "flex", alignItems: "center" }}>
            ...
            {button.params !== null && (
              <input
                ...
                value={inputValues[buttonIndex] || button.params}
